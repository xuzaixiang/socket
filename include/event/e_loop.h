...
EVENT_EXPORT e_loop_t *
e_loop_new(uint32_t flags DEFAULT(EVENT_LOOP_FLAG_AUTO_FREE));
 ...
 */
// NOTE: hloop_post_event is thread-safe, used to post event from other thread
// to loop thread.
EVENT_EXPORT void e_loop_post_event(e_loop_t *loop, e_event_t *ev);
// server
EVENT_EXPORT e_io_t *e_loop_create_tcp_server(e_loop_t *loop, const char *host,
                                              int port, e_accept_cb accept_cb);

EVENT_EXPORT e_io_t *e_loop_create_udp_server(e_loop_t *loop, const char *host,
                                              int port);

EVENT_EXPORT e_io_t *e_loop_create_udp_client(e_loop_t *loop, const char *host,
                                              int port);
