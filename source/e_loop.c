...
// hloop_process_ios -> hloop_process_timers -> hloop_process_idles ->
// hloop_process_pendings
static int e_loop_process_events(e_loop_t *loop) {
  ...
  //    uint64_t next_min_timeout =
  //    TIMER_ENTRY(loop->timers.root)->next_timeout; int64_t blocktime_us =
  //    next_min_timeout - hloop_now_hrtime(loop); if (blocktime_us <= 0) goto
  //    process_timers; blocktime = blocktime_us / 1000;
  //    ++blocktime;
  ...
  //  // printd("blocktime=%d nios=%d/%u ntimers=%d/%u nidles=%d/%u nactives=%d
  //  npendings=%d ncbs=%d\n",
...
void e_loop_post_event(e_loop_t *loop, e_event_t *ev) {
  ...
  //  nwrite = write(loop->eventfds[EVENTFDS_WRITE_INDEX], &count,
  //  sizeof(count));
